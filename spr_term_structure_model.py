...
def run_regression(df: pd.DataFrame) -> dict:
    ...
    # Parsimonious fallback: top 2 signals by economic priority; ensures df = n - 3 ≥ 1 when n ≥ 4
    FEATURES_PARSIMONIOUS = ["L1_us_delta_mmbbl", "L1_jp_delta_days"]

    model_df = df[["y"] + FEATURES_FULL].dropna()

    if len(model_df) < 2:
        print("  Insufficient data for regression.")
        return {}

    # Auto-select feature set: full model needs n > k+1 for identified OLS
    if len(model_df) <= len(FEATURES_FULL) + 1:
        FEATURES = [f for f in FEATURES_PARSIMONIOUS if f in model_df.columns and model_df[f].notna().any()]
        print(f"⚠  N={len(model_df)} ≤ k+1 — OLS saturated with full feature set.")
        print(f"   Using parsimonious model ({FEATURES}) for identified regression.")
        print(f"   Add historical brent_6m data (2020–2025) to reach N≥36 for full model.")
        model_df = df[["y"] + FEATURES].dropna()
    else:
        FEATURES = FEATURES_FULL
        if len(model_df) < 36:
            print(f"⚠  Only {len(model_df)} observations — target 36+ for reliable Newey-West inference.")
        ...
        rows = [
            {
                ...
                "variable": var,
                "coef": vals["coef"],
                "se": vals["se"] if np.isfinite(vals["se"]) else None,
                "t_stat": vals["t"] if np.isfinite(vals["t"]) else None,
                "p_value": vals["p"] if np.isfinite(vals["p"]) else None,
            }
            for var, vals in results.items()
        ]
        sb.table("spr_model_results").upsert(rows, on_conflict="run_date,variable").execute()
